...
NSString *const kExportDir= @"~/Desktop/YunQi_Builds/";//导出路径
NSString *const kAMExportIPA = @"导出IPA包 -> ~/Desktop/YunQi_Builds/";//导出菜单
...
- (void)generateIPA:(id)arg1 {
    ...
    dispatch_async(taskQueue, ^{
        ...
        @try {
            ...
            NSString *targetFilePath = [NSString stringWithFormat:@"%@%@-%@.ipa", kExportDir,targetFileName, dateString];
            //调 插件PackageApplication 执行打包操作。 这里我们也可以不用调插件，直接创建个PayLoad，把app包放进去然后 压缩就行
            //TODO ：这里我们可以把其他的脚步文件放到这里，比如执行上传脚本等
            NSString *commands = [NSString stringWithFormat:@"mkdir %@;xcrun -sdk iphoneos PackageApplication -v \"%@\" -o %@;open -R %@",
                                  kExportDir,
                                  decodeFilePath,
                                  targetFilePath,
                                  targetFilePath];
        ...
        @finally {
            ...
            //TODO: 这里导出后可以执行一些脚本，如上传到蒲公英、重签名等
            if (status == 0) {
